...
namespace Siglo21Desktop.Dao
{
    class ReservaDAO
    {
        ...
        public async Task<HttpResponseMessage> Save(Reserva obj)
        {
            string ruta = CommonEnums.CrudPath.ReservaCrud;
            var response = await Client.PutAsJsonAsync(ruta, obj);

            return response;
        }

        public async Task<HttpResponseMessage> Update(Reserva obj)
        {
            string ruta = CommonEnums.CrudPath.ReservaCrud;
        ...
        {
            string ruta = CommonEnums.CrudPath.ReservaCrud + id;
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var item = (await response.Content.ReadAsAsync<IEnumerable<Reserva>>()).FirstOrDefault();
                return item;
            }
